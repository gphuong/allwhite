...
class AllwhiteQueryBuilders {
    ...
    private static String buildSearch(BoolQueryBuilder query, List<String> filters, PageRequest pageable) {
        ...
        QueryBuilder filteredBuilder = QueryBuilders.filteredQuery(query, filterBuilder);

        addQuery(filteredBuilder, searchSourceBuilder);
    ...
    private static QueryBuilder matchProjectPages() {
        return QueryBuilders.termQuery("_type", SearchType.PROJECT_PAGE.toString());
    }

    private static QueryBuilder matchMarkedAsCurrent() {
        return QueryBuilders.termQuery("current", true);
    }

    private static QueryBuilder matchTitleContentAndAuthor(String queryTerm) {
