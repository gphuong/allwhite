...
public class SearchService {
    ...
    private JestResult execute(Action action) {
        try {
            ...
            if (!result.isSucceeded()) {
                logger.warn("Failed to execute Elastic Search action: " + result.getErrorMessage() + " " + result.getJsonString());
            }
            return result;
        } catch (Exception e) {
    ...
    public SearchResults search(String term, Pageable pageable, List<String> filter) {
        ...
        if (StringUtils.isEmpty(term)) {
            searchBuilder = AllwhiteQueryBuilders.forEmptyQuery(pageable, filter);
        } else {
            searchBuilder = AllwhiteQueryBuilders.fullTextSearch(term, pageable, filter);
        }
